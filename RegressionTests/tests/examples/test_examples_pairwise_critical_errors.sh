...
for score in $( seq 10 10 90 ); do
    $APPRAISE_MANAGE MakeAnnotation engdeu0701:55780fa8 Pairwise $score:$(( $score + 10 )) --errors serious-semantic-error1:serious-semantic-error2
done

# Export scores without timestamps and compare with the expected output
$APPRAISE_MANAGE ExportSystemScoresToCSV example7sqm | cut -f-8 -d, > $prefix.scores.csv
