...
class BasePairwiseAssessmentDocumentTask(BaseMetadata):
    """
    Abstract base PairwiseAssessmentDocumentTask
    """
...
class BasePairwiseAssessmentDocumentResult(BaseMetadata):
    """
    Abstract base PairwiseAssessmentDocumentResult
    """
...
class PairwiseAssessmentDocumentTask(BasePairwiseAssessmentDocumentTask):
    """
...
class PairwiseAssessmentDocumentResult(BasePairwiseAssessmentDocumentResult):
    """
    Models a pairwise assessment document evaluation result.
    """
...
@AnnotationTaskRegistry.register
class PairwiseAssessmentDocumentESATask(BasePairwiseAssessmentDocumentTask):
    """
    Models a pairwise assessment document evaluation with error span annotations task.
    """

    def completed_items_for_user(self, user):
        results = PairwiseAssessmentDocumentESAResult.objects.filter(
            task=self, activated=False, completed=True, createdBy=user
        ).values_list('item_id', flat=True)
    ...
    def next_item_for_user(self, user, return_completed_items=False):
        ...
        completed_items = 0
        for item in self.items.all().order_by('id'):
            result = PairwiseAssessmentDocumentESAResult.objects.filter(
                item=item, activated=False, completed=True, createdBy=user
            )
        ...
        if not next_item:
            LOGGER.info('No next item found for task {0}'.format(self.id))
            annotations = PairwiseAssessmentDocumentESAResult.objects.filter(
                task=self, activated=False, completed=True
            ).values_list('item_id', flat=True)
            uniqueAnnotations = len(set(annotations))
    ...
    def next_document_for_user(self, user, return_statistics=True):
        ...
        completed_blocks = PairwiseAssessmentDocumentESAResult.objects.filter(
            task=self,
            item__isCompleteDocument=True,
            completed=True,
            createdBy=user,
    ...
    def get_results_for_each_item(self, block_items, user):
        ...
        for item in block_items:
            result = (
                PairwiseAssessmentDocumentESAResult.objects.filter(
                    item__id=item.id,
                    completed=True,
                    createdBy=user,  # TODO: is passing user as an argument needed?
                    task=self,
    ...
    def import_from_json(cls, campaign, batch_user, batch_data, max_count):
        """
        Creates new PairwiseAssessmentDocumentESATask instances based on JSON input.
        """
        batch_meta = batch_data.metadata
        batch_name = batch_data.dataFile.name
        batch_file = batch_data.dataFile
        ...
        for batch_task in batch_json:
            ...
            new_task = PairwiseAssessmentDocumentESATask(
                campaign=campaign,
                requiredAnnotations=batch_task['task']['requiredAnnotations'],
                batchNo=batch_task['task']['batchNo'],
                batchData=batch_data,
...
class PairwiseAssessmentDocumentESAResult(BasePairwiseAssessmentDocumentResult):
    """
    Models a pairwise assessment document evaluation with error span annotations result.
    """
    ...
    def get_system_annotations(cls):
        ...
        for result in qs.values_list(*value_names):
            ...
            mqm2 = result[8]
            system_scores[marketID].append(
                (systemID, annotatorID, segmentID, score1, score2, mqm1, mqm2)
            )
    ...
    def dump_all_results_to_csv_file(cls, csv_file):
        ...
        x = system_scores
        s = [
            'taskID,segmentID,username,email,groups,system1ID,score1,system2ID,score2,startTime,endTime,durationInSeconds,itemType,campaignName,documentID,isCompleteDocument,mqm1,mqm2'
        ]
        for l in x:
            for i in x[l]:
                s.append(','.join([str(a) for a in i]))
    ...
    def write_csv(cls, srcCode, tgtCode, domain, csvFile, allData=False):
        x = cls.get_csv(srcCode, tgtCode, domain)
        s = [
            'username,email,segmentID,score1,score2,durationInSeconds,itemType,documentID,isCompleteDocument,mqm1,mqm2'
        ]
        if allData:
            s[0] = 'systemID,' + s[0]
